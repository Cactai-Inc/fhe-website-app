...
## RESUME
Role / thread   FHE-MGMT-GRANTS · wt-1 · bundle/grants @ 1e421e45 (+ this commit); origin/main c12d711e, NOT merged in yet
DONE            -A merged · escalation 1 ruled + recorded · -B spec amended on the ruling · **-B BUILT AND APPLIED TO PRODUCTION 11:31 PDT** (195 ACL revokes) · MGMT's own merge-time re-proof run 12:20:15 PDT — 10 spot-checks, all HOLD · -B's three questions ANSWERED (below) · spec §4's dangling "exception" struck · **-V dispatched to wt-11**
IN FLIGHT       FHE-TASK-GRANTS-V verifying (Opus · HIGH · thinking ON · wt-11 · reads task/grants-b @ 7f2b36ff)
NEXT            on -V's HOLDS: merge task/grants-b into bundle/grants (NOT main) → gates → `## VALIDATION` on TASK-GRANTS-B-REPORT.md + TASK-LEDGER.md line → push bundle/grants → ORCH merges the branch WHOLE → -W walks the deploy → item 7 → bundle report
DECIDED         **Q1 merge order:** -B merges into bundle/grants only; ORCH merges bundle/grants to main as ONE unit, so main never holds the migration beside the pre-ruling spec. No re-ordering needed — this IS the ruled lane (MGMT-ROLE §8). **Q2 default privileges:** OUT of this bundle's ownership (a schema default is not an ACL on a named function) → routed to ORCH with a recommendation, NOT built. **Q3 the "exception":** none was ever meant; group S is two REVOKEs and no GRANT, service_role keeps EXECUTE by not being revoked (proven 12:20). Phrase struck from the spec.
BLOCKED         nothing
DO NOT          ⚠️ **do not read the -B spec from task/grants-b's tree — that branch carries the PRE-RULING copy that still says KEEP open_gift** (main does too; the ruling lives only on bundle/grants). · do not merge task/grants-b to main · do not build ALTER DEFAULT PRIVILEGES here · do not touch bodies · do not `git checkout <ref> --` with no path in wt-1
