...
| `_waiting_items()` (SQL, 9,640 chars, `side ∈ {'you','client'}`, ranked) + `dash_waiting_on_you()` / `dash_waiting_on_clients()` (376/379 chars, thin wrappers) | Live in production, **no caller in `src/`** (DB-MAP), **no migration on `main` creates them** (applied from the unmerged `b9bc9edc` WaitingZones branch; `grep -rl _waiting_items supabase/migrations` = 0). | **REUSED, not retired.** `side='you'|'client'` IS DASHFEED §4b's YOURS · THEIRS ask-axis, already written. They become the source of the action-cluster elements `A_YOURS` / `A_THEIRS` (§3.2). ⚠️ The chunk that registers those elements (BOARDS, chunk D) **lands their `CREATE OR REPLACE` in a migration on `main`** so the repo creates what production runs. Until then they stay as they are (D32). |
...
- **The ask axis for actions maps onto `_waiting_items.side`**: `'you'` → `yours`, `'client'` → `theirs`; `today` is time-bound work (C1's rows). The two action elements `A_YOURS` / `A_THEIRS` (source `dash_waiting_on_you` / `dash_waiting_on_clients`) are the reference implementation; a domain-specific action element (C3 money-to-confirm, C4 people waiting) declares its `ask` the same way.
