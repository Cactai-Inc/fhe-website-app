...
```
**MGMT (Opus · HIGH · thinking ON — D45):** `FHE-MGMT-<BUNDLE>` + `Read <abs>/docs/orch/BUNDLE-<NAME>.md and run it.` + the Worktree/hand-back line.
...
**D45 (owner, 2026-09-03): Fable is NEVER a default. Default for MGMT, DSNR, VRFY, WALKR and builds is Opus · HIGH · thinking ON; idiom repetition → Sonnet · MEDIUM · thinking ON. Fable · HIGH only when the OWNER names the thread, ONE at a time** (30% of the weekly Fable allowance went in 9 hours with seven Fable threads up). Bring him the candidate with why the ground is convoluted; he says yes or no. Give Fable outcome + incumbents + rulings + traps, not a route.
