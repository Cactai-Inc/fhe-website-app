...
| # | Item | Status |
|---|---|---|
| I1 | Sidebar collapse/expand toggle: staff/admin only — removed entirely for USER accounts | **PARTIAL — code-complete, browser pending.** ClientRail's pin/hover toggle removed; USER sidebar is now fixed at 240px. Staff rail had no such toggle already, so no account type has one now. See `TASK-I-REPORT.md`. |
