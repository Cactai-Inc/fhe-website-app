...
| F3 | ~~Visit-day release kiosk + participant flow~~ | — | ~~`/release`, `/docs/release-participant`~~ | executed releases RETAINED (D32) | 🔒 **RETIRED 2026-09-01 (TASK-SIGNFLOW-D).** Owner: *"we dont use docs/release-participant nor /release … the /sign/ flow should be the single pathway we use"* and *"we dont have a situation where a person without an account signs documents on an ipad or any other way."* **Routes removed from `App.tsx`; `api/sign-release.ts`, `Release.tsx`, `DocsParticipantFlow.tsx` deleted; anon EXECUTE revoked on `sign_release`/`sign_general_release` (`20260902T0010`).** ⚠️ **Its "35 delivery rows" was WRONG — the measured figure was 28.** **Measured before removal: 40 signatures, 10 real signers, 2026-07-13 → 2026-08-15, ALL through `/docs/release-participant` (`/release` itself produced ZERO).** **`/sign/rider` assigns the identical four-document set; F2 is the incumbent.** |
