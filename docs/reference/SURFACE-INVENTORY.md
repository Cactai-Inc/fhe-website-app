...
| ~~`/release`, `/release/:releaseKey`~~ | ~~Release.tsx~~ (deleted) | NONE | 🔒 **RETIRED 2026-09-01 (TASK-SIGNFLOW-D)** — route removed from `App.tsx`, page file deleted, anon EXECUTE revoked on `sign_release`/`sign_general_release` (`20260902T0010`). Owner: *"we dont have a situation where a person without an account signs documents on an ipad or any other way."* **The incumbent is `/sign` and its four funnels.** **The row is kept, not deleted, so the next reader does not re-discover a deliberately removed surface.** ⚠️ **Measured before removal: it produced ZERO signatures ever** | retired | RETIRED | — | — | — | none |
| ~~`/docs/release-participant`~~ | ~~DocsParticipantFlow.tsx~~ (deleted) | NONE | 🔒 **RETIRED 2026-09-01 (TASK-SIGNFLOW-D)** — route removed from `App.tsx`, page file deleted, anon EXECUTE revoked on `sign_release`/`sign_general_release` (`20260902T0010`). Owner: *"we dont have a situation where a person without an account signs documents on an ipad or any other way."* **The incumbent is `/sign` and its four funnels.** **The row is kept, not deleted, so the next reader does not re-discover a deliberately removed surface.** ⚠️ **Measured before removal: 40 signatures from 10 real signers, 2026-07-13 → 2026-08-15 — every kiosk signature came through HERE. The executed documents are RETAINED (D32).** | retired | RETIRED | — | — | — | none |
