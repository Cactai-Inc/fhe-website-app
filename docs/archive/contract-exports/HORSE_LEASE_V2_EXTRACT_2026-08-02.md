...
Generated 2026-08-02 04:24:12 UTC from the live database (project `lrstswfxfsezdmvkvukc`),
...
live lease template, in render order. Regenerated companion to the prior extract
in `docs/contract-exports/HORSE_LEASE_V2_EXTRACT_2026-08-01.md` (now archived) —
clause keys and field keys match except where this run's CHANGED-CONTENT list
...
**CONDITIONAL** — shows when: TXN.PAYMENT_METHODS = 
...
**CONDITIONAL** — shows when: TXN.LESSOR_PAYMENT_METHODS = 
...
**CONDITIONAL** — shows when: TXN.EVALUATION_ENABLED = REQUESTED or REQUIRED AND TXN.EVAL_PERIOD_TYPE = INCLUDED AND TXN.EVAL_INCLUDED_LENGTH = 
...
**CONDITIONAL** — shows when: TXN.EVALUATION_ENABLED = REQUESTED or REQUIRED AND TXN.EVAL_PERIOD_TYPE = FIXED AND TXN.EVAL_FIXED_LENGTH = 
...
**CONDITIONAL** — shows when: TXN.PERMITTED_ACTIVITIES = 
...
**CONDITIONAL** — shows when: LESSEE.PARTY_TYPE = INDIVIDUAL AND TXN.PERMITTED_ACTIVITIES = 
...
**CONDITIONAL** — shows when: LESSEE.PARTY_TYPE = ENTITY AND TXN.PERMITTED_ACTIVITIES = 
...
**CONDITIONAL** — shows when: LESSEE.PARTY_TYPE = (empty) AND TXN.PERMITTED_ACTIVITIES = 
...
### RESTRICT.JUMP_ON *(no heading set)*

`RESTRICT.JUMP_ON`

**CONDITIONAL** — shows when: TXN.PERMITTED_ACTIVITIES =  AND TXN.JUMP_OMIT = NO or (empty)
...
### RESTRICT.JUMP_OFF *(no heading set)*

`RESTRICT.JUMP_OFF`

**CONDITIONAL** — shows when: TXN.PERMITTED_ACTIVITIES =  AND TXN.JUMP_OMIT = YES
...
### RESTRICT.COMP_ON *(no heading set)*

`RESTRICT.COMP_ON`

**CONDITIONAL** — shows when: TXN.PERMITTED_ACTIVITIES =  AND TXN.COMP_OMIT = NO or (empty)
...
### RESTRICT.COMP_OFF *(no heading set)*

`RESTRICT.COMP_OFF`

**CONDITIONAL** — shows when: TXN.PERMITTED_ACTIVITIES =  AND TXN.COMP_OMIT = YES
...
### RESTRICT.TRAIL_ON *(no heading set)*

`RESTRICT.TRAIL_ON`

**CONDITIONAL** — shows when: TXN.PERMITTED_ACTIVITIES =  AND TXN.TRAIL_OMIT = NO or (empty)
...
### RESTRICT.TRAIL_OFF *(no heading set)*

`RESTRICT.TRAIL_OFF`

**CONDITIONAL** — shows when: TXN.PERMITTED_ACTIVITIES =  AND TXN.TRAIL_OMIT = YES
...
**CONDITIONAL** — shows when: TXN.ADDITIONAL_ACTIVITIES = 
...
### PROHIBITED.OTHER_NOTE *(no heading set)*
...
### PROHIBITED.OTHERS_OTHER *(no heading set)*

`PROHIBITED.OTHERS_OTHER`

**CONDITIONAL** — shows when: TXN.OTHERS_ALLOWED = 
...
**CONDITIONAL** — shows when: TXN.RIDER_AIDS = 
