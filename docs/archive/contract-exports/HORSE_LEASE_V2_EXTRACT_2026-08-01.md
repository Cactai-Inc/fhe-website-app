...
Generated 2026-08-02 01:06:18 UTC from the live database (project `lrstswfxfsezdmvkvukc`),
reflecting migration head `20260801040000_clause_gate_close_and_provision_drop.sql`.

Every section, clause, field, option list, helper text and conditional in the
live lease template, in render order. Regenerated companion to the archived
extract in `docs/archive/contract-exports/` — clause keys and field keys match.
...
- **Year foaled** — `HORSE.AGE_DOB` · input: text · owner: LESSOR
...
**CONDITIONAL** — shows when: LESSOR.PARTY_TYPE is unset
...
**CONDITIONAL** — shows when: LESSEE.PARTY_TYPE is unset
...
- **First monthly payment date** — `TXN.MONTHLY_START` · input: date · owner: DEAL
    - **CONDITIONAL** — shows when: TXN.LEASE_FEE_TYPE = FEE
...
- **Evaluation period** — `TXN.EVAL_PERIOD_TYPE` · input: select · owner: DEAL
    ...
    - **CONDITIONAL** — shows when: TXN.EVALUATION_ENABLED is one of (REQUESTED, REQUIRED)

### Evaluation Period Details

`EVALUATION.DATES_INCLUDED`

**CONDITIONAL** — shows when: TXN.EVALUATION_ENABLED is one of (REQUESTED, REQUIRED) AND TXN.EVAL_PERIOD_TYPE = INCLUDED AND TXN.EVAL_INCLUDED_LENGTH >= 1
...
**CONDITIONAL** — shows when: TXN.EVALUATION_ENABLED is one of (REQUESTED, REQUIRED) AND TXN.EVAL_PERIOD_TYPE = FIXED AND TXN.EVAL_FIXED_LENGTH >= 1
...
**CONDITIONAL** — shows when: LESSEE.PARTY_TYPE = INDIVIDUAL AND TXN.PERMITTED_ACTIVITIES includes LESSONS
...
**CONDITIONAL** — shows when: LESSEE.PARTY_TYPE = ENTITY AND TXN.PERMITTED_ACTIVITIES includes LESSONS
...
**CONDITIONAL** — shows when: LESSEE.PARTY_TYPE is unset AND TXN.PERMITTED_ACTIVITIES includes LESSONS
...
Every `{{TOKEN}}` appearing in a clause body above (109 distinct),
with the field def backing it where one exists.

- `{{DOC.EFFECTIVE_DATE}}` — *(no field def; filled from record data or signature)*
- `{{HORSE.AGE_DOB}}` — Year foaled · input: text
- `{{HORSE.BREED}}` — Breed · input: select
- `{{HORSE.COLOR}}` — Color · input: select
- `{{HORSE.CURRENT_LOCATION}}` — Facility · input: location
- `{{HORSE.FAIR_MARKET_VALUE}}` — Fair market value · input: currency
- `{{HORSE.FARRIER_NAME}}` — Farrier · input: text
- `{{HORSE.FARRIER_PHONE}}` — Farrier phone · input: text
- `{{HORSE.MARKINGS}}` — Markings · input: text
- `{{HORSE.MICROCHIP}}` — Microchip # · input: text
- `{{HORSE.PASSPORT_NUMBER}}` — Passport # · input: text
- `{{HORSE.REGISTERED_NAME}}` — Registered name · input: text · required
- `{{HORSE.REGISTRATION_NUMBER}}` — Registration number · input: text
- `{{HORSE.SEX}}` — Sex · input: select
- `{{HORSE.VET_ADDRESS}}` — Address · input: text
- `{{HORSE.VET_BUSINESS}}` — Practice · input: text
- `{{HORSE.VET_NAME}}` — Veterinarian · input: text
- `{{HORSE.VET_PHONE}}` — Veterinarian phone · input: text
- `{{LESSEE.ADDRESS}}` — *(no field def; filled from record data or signature)*
- `{{LESSEE.EMAIL}}` — *(no field def; filled from record data or signature)*
- `{{LESSEE.FULL_NAME}}` — *(no field def; filled from record data or signature)*
- `{{LESSEE.PHONE}}` — *(no field def; filled from record data or signature)*
- `{{LESSEE.PRINTED_NAME}}` — *(no field def; filled from record data or signature)*
- `{{LESSOR.ADDRESS}}` — *(no field def; filled from record data or signature)*
- `{{LESSOR.EMAIL}}` — *(no field def; filled from record data or signature)*
- `{{LESSOR.FULL_NAME}}` — *(no field def; filled from record data or signature)*
- `{{LESSOR.PHONE}}` — *(no field def; filled from record data or signature)*
- `{{LESSOR.PRINTED_NAME}}` — *(no field def; filled from record data or signature)*
- `{{SIG.LESSEE.DATE}}` — *(no field def; filled from record data or signature)*
- `{{SIG.LESSEE.NAME}}` — *(no field def; filled from record data or signature)*
- `{{SIG.LESSOR.DATE}}` — *(no field def; filled from record data or signature)*
- `{{SIG.LESSOR.NAME}}` — *(no field def; filled from record data or signature)*
- `{{TXN.ADDITIONAL_ACTIVITIES}}` — Additional permitted activities · input: buttons
- `{{TXN.ADDITIONAL_ACTIVITIES_OTHER}}` — Other additional permitted activity · input: text
- `{{TXN.ADDITIONAL_TERMS}}` — Add additional terms · input: add_text
- `{{TXN.BEHAVIOR_EXCEPTIONS}}` — Known behavior exceptions · input: longtext
- `{{TXN.CARD_PROCESSOR}}` — Card processor & instructions · input: longtext
- `{{TXN.CAUSE_TERM_NOTICE_DAYS}}` — Days notice · input: number
- `{{TXN.COMPETITION_EXPENSES}}` — Competition expenses · input: select
- `{{TXN.COMPETITION_WINNINGS}}` — Competition winnings · input: select
- `{{TXN.COMP_OMIT}}` — No competition restrictions · input: certify
- `{{TXN.COMP_RESTRICTION}}` — Competition restriction · input: text
- `{{TXN.CONDITION_EXCEPTIONS}}` — Known condition exceptions · input: longtext
- `{{TXN.CO_OWNERS}}` — Co-owner(s) · input: contacts_list
- `{{TXN.DAYS_USED}}` — Reserved days of use · input: week_grid
- `{{TXN.EVAL_FIXED_FEE}}` — Evaluation period fee amount · input: currency
- `{{TXN.EVAL_FIXED_LENGTH}}` — Length · input: number
- `{{TXN.EVAL_FIXED_UNIT}}` — Unit · input: select
- `{{TXN.EVAL_INCLUDED_LENGTH}}` — Length · input: number
- `{{TXN.EVAL_INCLUDED_UNIT}}` — Unit · input: select
- `{{TXN.FARRIER_ARRANGE}}` — Party responsible for arranging · input: select
- `{{TXN.FARRIER_COST_PARTY}}` — Party responsible for costs · input: select
- `{{TXN.GL_DED_RESP}}` — Deductible responsibility (Lessee-responsibility claims) · input: select · required
- `{{TXN.GL_DED_RESP_SPLIT_LESSEE}}` — Split — % paid by Lessee · input: text
- `{{TXN.GL_DED_RESP_SPLIT_LESSOR}}` — Split — % paid by Lessor · input: text
- `{{TXN.GL_LESSEE_STATUS}}` — Lessee · input: select · required
- `{{TXN.GL_LESSOR_STATUS}}` — Lessor · input: select · required
- `{{TXN.HAS_OWNERSHIP_LIMITS}}` — Any limitations on ownership? · input: yesno
- `{{TXN.JUMP_DAYS_PER_WEEK}}` — Days per week · input: number
- `{{TXN.JUMP_MAX_HEIGHT}}` — Maximum height · input: text
- `{{TXN.JUMP_OMIT}}` — No jumping restrictions · input: certify
- `{{TXN.JUMP_SUPERVISION}}` — Only under trainer supervision? · input: yesno
- `{{TXN.LEASE_END}}` — Lease end date · input: date
- `{{TXN.LEASE_FEE}}` — Lease fee · input: fee_schedule
- `{{TXN.LEASE_PURPOSE}}` — Purpose of the lease · input: select · required
- `{{TXN.LEASE_START}}` — Lease start date · input: date · required
- `{{TXN.LEASE_TERM_TYPE}}` — Term type · input: select · required
- `{{TXN.LEASE_TYPE}}` — Lease type · input: select · required
- `{{TXN.LESSEE_TERM_NOTICE_DAYS}}` — Days notice · input: number
- `{{TXN.LESSONS_ENTITY_PERMITTED}}` — Lessee permitted to provide riding lessons? · input: yesno
- `{{TXN.LESSONS_REQUIRED}}` — Lessee required to take lessons? · input: yesno
- `{{TXN.LESSOR_CARD_PROCESSOR}}` — Card processor & instructions · input: longtext
- `{{TXN.LESSOR_PAYMENT_METHODS}}` — Accepted payment methods · input: buttons
- `{{TXN.MEDICATIONS}}` — Medications and supplements · input: med_schedule
- `{{TXN.MED_DED_RESP}}` — Deductible responsibility · input: select · required
- `{{TXN.MED_DED_RESP_SPLIT_LESSEE}}` — Deductible split — paid by Lessee · input: text
- `{{TXN.MED_DED_RESP_SPLIT_LESSOR}}` — Deductible split — paid by Lessor · input: text
- `{{TXN.MED_LESSEE_STATUS}}` — Lessee · input: select · required
- `{{TXN.MED_LESSOR_STATUS}}` — Lessor · input: select · required
- `{{TXN.MORT_DED_RESP}}` — Deductible responsibility · input: select · required
- `{{TXN.MORT_DED_RESP_SPLIT_LESSEE}}` — Deductible split — paid by Lessee · input: text
- `{{TXN.MORT_DED_RESP_SPLIT_LESSOR}}` — Deductible split — paid by Lessor · input: text
- `{{TXN.MORT_LESSEE_STATUS}}` — Lessee · input: select · required
- `{{TXN.MORT_LESSOR_STATUS}}` — Lessor · input: select · required
- `{{TXN.NEW_LOCATION}}` — Location during lease term · input: location
- `{{TXN.OFFSITE_TRANSPORT}}` — Offsite transport · input: select
- `{{TXN.OTHERS_ALLOWED}}` — Others allowed to ride · input: buttons
- `{{TXN.OTHERS_ALLOWED_OTHER}}` — Other persons allowed · input: text
- `{{TXN.OWNERSHIP_LIMITATIONS}}` — Ownership limitations · input: longtext
- `{{TXN.OWNER_TERM_NOTICE_DAYS}}` — Days notice · input: number
- `{{TXN.PAYMENT_METHODS}}` — Accepted payment methods · input: buttons
- `{{TXN.PERMITTED_ACTIVITIES}}` — Permitted activities · input: buttons · required
- `{{TXN.PERMITTED_RESTRICTIONS}}` — Add Restrictions · input: add_text
- `{{TXN.PROTECTIVE_EQUIPMENT}}` — Protective equipment · input: buttons
- `{{TXN.PROTECTIVE_REQUIRED}}` — Horse must wear protective equipment · input: yesno
- `{{TXN.RENEWAL_TERMS}}` — Renewal terms · input: longtext
- `{{TXN.RIDER_AIDS}}` — Prohibited rider aids · input: buttons
- `{{TXN.RIDER_AIDS_OTHER}}` — Other prohibited rider aid · input: text
- `{{TXN.SCHEDULE_TERMS}}` — Additional schedule terms · input: longtext
- `{{TXN.TACK_PROHIBITED}}` — Is Lessee prohibited from using certain tack or equipment? · input: reveal_text
- `{{TXN.TRAIL_OMIT}}` — No trail-riding restrictions · input: certify
- `{{TXN.TRAIL_RESTRICTION}}` — Trail-riding restriction · input: text
- `{{TXN.TRAINER_EVAL_CHOICE}}` — Professional suitability evaluation · input: buttons
- `{{TXN.TRAINER_EXERCISE_ARRANGE}}` — Party responsible for arranging · input: select
- `{{TXN.TRAINER_EXERCISE_COST}}` — Party responsible for costs · input: select
- `{{TXN.TRAINER_EXERCISE_SPLIT_PCT}}` — Lessee's share of the cost · input: percent
- `{{TXN.VET_ARRANGE}}` — Party responsible for arranging · input: select
- `{{TXN.VET_CHECK_CHOICE}}` — Pre-lease veterinary examination · input: buttons
- `{{TXN.VET_COST_PARTY}}` — Party responsible for costs · input: select

---

Counts: 23 sections · 135 clauses · 108 field defs · 109 distinct tokens.
