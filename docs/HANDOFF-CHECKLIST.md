...
**Do not write the handoff yet.** This file exists to (a) preserve the owner's instruction
...
- [ ] 1. Explain the orchestrator's **role**
- [ ] 2. Explain the **process workflow** we are running
- [ ] 3. **How you choose what to run** — one at a time, from orchestrator to owner
- [ ] 4. **How you compose what you write** for each task thread
- [ ] 5. **What the task FILE contains**
- [ ] 6. **What the PROMPT contains**
- [ ] 7. **How you review the output**
- [ ] 8. Handling a completed thread — **success**
- [ ] 9. Handling a completed thread — **failure**
- [ ] 10. Handling a completed thread — **everything in between**
- [ ] 11. **How you manage the repo**
- [ ] 12. **Merges**
- [ ] 13. **Pushes**
- [ ] 14. Communication — **planning**
- [ ] 15. Communication — **organizing**
- [ ] 16. Communication — **changes**
- [ ] 17. Communication — **errors**
- [ ] 18. Communication — **new requests**
...
- [ ] **S1. Thread naming.** The tab title is auto-generated and drops a `THREAD ID:` header
      ...
      repeats all three.
- [ ] **S2. Worktree rules.** One worktree per thread, off `origin/main`, at
...
- [ ] **S3. Verify before asserting.** The single rule that has caught the most. Includes:
...
- [ ] **S4. Standing constraints that outlive any one task.** Sarah's document
...
- [ ] **S5. File ownership between concurrent threads.** Two threads holding `AppLayout.tsx`
      is how work gets clobbered. How ownership is assigned, published and released.
- [ ] **S6. Migration discipline.** Dry-run in `BEGIN…ROLLBACK` with raw output, apply,
...
- [ ] **S7. Where the settled decisions live** and that they are not re-litigated —
      `CLAUDE.md` D1–D9, the owner rulings inside task docs.
- [ ] **S8. What the orchestrator must NOT do.** Chief among them: **do not make design
...
- [ ] **S9. Correct the artifact, not just the chat.** When the owner corrects something, the
      fix goes into the doc that carries the error, or the next thread inherits it.
- [ ] **S10. The index problem.** Workstreams live in separate unindexed documents — the
...
- [ ] **S11. Deploy model.** Push to `main` auto-deploys. There is no separate deploy step,
      so a push is a release and an unverified visual change reaches the owner immediately.
- [ ] **S12. Task sizing and splitting.** When a task is too large or trips a model safeguard,
...
- [ ] **S13. Model, effort and thinking settings** per task, and the basis for choosing them.
- [ ] **S14. Recovering stranded work.** What to do when a thread's branch is unpushed and its
      clone is gone — the NULLUID case, recovered by hand from disk.
- [ ] **S15. Owner communication style.** Wants status, not narrative: what is finalised, what
