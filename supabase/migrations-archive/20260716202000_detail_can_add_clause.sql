-- contract_document_detail now returns can_add_clause in party_controls so the
-- 'Can add new clauses' checkbox reflects its saved state (was dropped on read-back).
