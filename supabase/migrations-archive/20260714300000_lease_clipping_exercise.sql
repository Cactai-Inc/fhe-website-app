/*
  # Lease contract — two new care terms: hair CLIPPING + partial-lease EXERCISE

  Adds two DEAL-owned fields to the HORSE_LEASE contract and weaves them into
  §9 CARE AND EXERCISE of the body:
    - TXN.CLIPPING_RESPONSIBILITY — who pays for / arranges the horse's hair
      (body) clipping.
    - TXN.EXERCISE_RESPONSIBILITY — who is responsible for exercising the horse;
      in a partial lease, which participant(s).

  Recreates start_lease_contract with the two fields appended to its seed array
  (seed_contract_fields upserts by (document_id, field_key)), patches the
  HORSE_LEASE body to add the two token lines, and re-derives template_tokens.
*/

DROP FUNCTION IF EXISTS start_lease_contract(uuid, uuid, uuid, text);
...
BEGIN
  ...
  -- The responsible party authors the deal terms → they are the originator.
  ...
  v_n := seed_contract_fields(v_doc, jsonb_build_array(
    ...
    -- ── Operational scheduling terms (drive the leased-horse calendar) ──
    jsonb_build_object('field_key','TXN.PAYMENT_OPTIONS','label','Payment Options (one per line: amount — describe)','section','Payment','owner_role','DEAL','value_type','longtext','sort_order',415),
    ...
    jsonb_build_object('field_key','TXN.LESSONS_BEGINNER','label','Lessons/Day — Beginner','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',820),
    jsonb_build_object('field_key','TXN.LESSONS_INTERMEDIATE','label','Lessons/Day — Intermediate','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',830),
    jsonb_build_object('field_key','TXN.LESSONS_ADVANCED','label','Lessons/Day — Advanced','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',840),
    ...
    -- ── Owner-only permissions (lessor's sole discretion) ──
...
REVOKE ALL ON FUNCTION start_lease_contract(uuid, uuid, uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION start_lease_contract(uuid, uuid, uuid, text) TO authenticated, service_role;

-- ── Body: add the two lines under §9 CARE AND EXERCISE, right after the routine
--    care/exercise responsibility line. Surgical replace (no full-body reload). ──
UPDATE contract_templates
   SET body = replace(
     body,
     E'Routine care and exercise responsibility (feeding, daily care, exercise, scheduling, and following facility rules): {{TXN.CARE_RESPONSIBILITY}}',
 ...
 WHERE template_key = 'HORSE_LEASE'
   AND body LIKE '%{{TXN.CARE_RESPONSIBILITY}}%'
   AND body NOT LIKE '%{{TXN.EXERCISE_RESPONSIBILITY}}%';

-- ── Re-derive template_tokens for HORSE_LEASE from the (now patched) body,
--    matching the canonical reload (namespace/field/kind all populated). ──
