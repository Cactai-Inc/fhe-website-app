/*
  # Mandatory rider protective-equipment clause (owner)

  §12 protective equipment becomes a FIXED, mandatory clause — helmet/boots/pants
  required, rider provides them, declining voids the lease, and injury from not
  wearing them is rider negligence releasing all parties. Replaces the two
  fillable protective-equipment lines (the tack/equipment fields stay). The now-
  obsolete TXN.PROTECTIVE_EQUIPMENT / _PROVIDER fields are dropped from the lease
  seed so they aren't captured-but-unused. (Typos in the source text corrected.)
*/
