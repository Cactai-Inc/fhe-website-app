-- Two more composer fixes from the permitted-use review:
--   #1 multi-select (checkbox) tokens rendered the raw CSV of stored CODES
--      ("LESSONS,JUMPING,COMPETITIONS,TRAIL") instead of the option LABELS.
--      token_display_value now maps EACH comma-separated value to its label and
--      rejoins with ", " → "Riding Lessons, Jumping, Competitions, Trail Riding".
--   #2 an authoring-only gate clause (empty body, no heading — e.g. the evaluation
--      enable prompt) still consumed a clause NUMBER, producing "7. …" then a
--      "7.2" with no 7.1. A clause that contributes no heading and no body lines
--      no longer advances the clause counter.

-- multi-value aware label mapping
...
AS $$
  SELECT CASE
    ...
    -- comma-separated multi-select → map each value to its label, keep order
...
DECLARE
  ...
  v_out        text[] := '{}';
  v_sec_buf    text[];
  v_cl_buf     text[];          -- lines produced by the CURRENT clause
  v_sec        record;
  v_cl         record;
  v_sec_no     int := 0;
  v_cl_no      int;
...
BEGIN
  ...
  FROM (
    SELECT field_key, jsonb_object_agg(opt->>'value', opt->>'label') AS vmap
  ...
  LOOP
    ...
    LOOP
      ...
      -- build this clause's body lines into a local buffer FIRST, so we know
      -- whether it actually produces output before assigning it a number.
      v_cl_buf := '{}';
      IF v_body <> '' THEN
      ...
      -- a clause with NO heading and NO produced body is authoring-only (e.g. an
      -- enable gate) — it contributes nothing and must not consume a number.
      IF (v_cl.heading IS NULL OR v_cl.heading = '')
         AND coalesce(array_length(v_cl_buf,1),0) = 0 THEN
        CONTINUE;
      END IF;
