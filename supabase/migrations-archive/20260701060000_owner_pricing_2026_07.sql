...
SELECT (SELECT id FROM organizations ORDER BY created_at LIMIT 1),
       'horse', 'Horse Exercise', 'Lunging sessions to keep your horse in consistent work', 'horse-exercise', true, 45
...
BEGIN
  ...
  DELETE FROM offering_tiers WHERE offering_id = v_id;
  INSERT INTO offering_tiers (org_id,offering_id,label,description,price_amount,price_unit,note,is_popular,sort_order) VALUES
    (v_org,v_id,'Exercise Session','Single lunging session',55,'session',NULL,false,1),
  ...
  DELETE FROM offering_tiers WHERE offering_id = v_id;
  INSERT INTO offering_tiers (org_id,offering_id,label,description,price_amount,price_unit,note,is_popular,sort_order) VALUES
    (v_org,v_id,'Turnout Session','Single turnout session — the lighter exercise option',25,'session',NULL,false,1),
