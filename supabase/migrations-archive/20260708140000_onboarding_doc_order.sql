/*
  # Spine 2d — onboarding doc order Rules→Policies (owner C2)
  Swap COMPANY_POLICIES/FACILITY_RULES so the signing sequence is
  FACILITY_RULES → COMPANY_POLICIES → RELEASE_PARTICIPANT → HUMAN_EMERGENCY_MEDICAL,
  matching the /docs/release-participant flow. Reissues both onboarding RPCs
  with only the array_position priority swapped.
*/
