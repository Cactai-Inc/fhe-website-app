-- Seed the two new location fields in start_lease_contract so they get inputs:
-- HORSE.HOME_LOCATION (fills from the horse record) and TXN.CONTRACT_LOCATIONS
-- (longtext for one-or-more locations w/ optional dates, DEAL-owned so either
-- party can set it in creation per controls).
...
BEGIN
  ...
  -- The responsible party authors the deal terms Ã¢ÂÂ they are the originator.
  ...
  v_n := seed_contract_fields(v_doc, jsonb_build_array(
    ...
    -- Ã¢ÂÂÃ¢ÂÂ Operational scheduling terms (drive the leased-horse calendar) Ã¢ÂÂÃ¢ÂÂ
    jsonb_build_object('field_key','TXN.PAYMENT_OPTIONS','label','Payment Options (one per line: amount Ã¢ÂÂ describe)','section','Payment','owner_role','DEAL','value_type','longtext','sort_order',415),
    ...
    jsonb_build_object('field_key','TXN.LESSONS_BEGINNER','label','Lessons/Day Ã¢ÂÂ Beginner','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',820),
    jsonb_build_object('field_key','TXN.LESSONS_INTERMEDIATE','label','Lessons/Day Ã¢ÂÂ Intermediate','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',830),
    jsonb_build_object('field_key','TXN.LESSONS_ADVANCED','label','Lessons/Day Ã¢ÂÂ Advanced','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',840),
    ...
    -- Ã¢ÂÂÃ¢ÂÂ Owner-only permissions (lessor's sole discretion) Ã¢ÂÂÃ¢ÂÂ
