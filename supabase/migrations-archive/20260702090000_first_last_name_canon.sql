/*
  # first_name + last_name canon — contacts.full_name REMOVED (owner 2026-07-02)

  Owner directive A: contacts carry first_name + last_name ONLY. Every official
  surface (contract merges, printed names, signed kiosk documents) renders
  first_name || ' ' || last_name (single space, trimmed when a part is missing);
  casual surfaces use first_name.

  Order of operations (shipped migrations untouched — heads re-issued in full):
    1. Backfill first/last from full_name (split on the FIRST space; a
       single-token name lands in first_name only).
    2. ensure_contact_for_profile — re-issued without the full_name column
       (fallback identity for a nameless profile now lands in first_name).
    3. generate_document v7 — FULL v6 body (20260701080000) with the party arm
       reading the first/last concatenation for FULL_NAME / PRINTED_NAME.
    4. sign_release — signature CHANGES: p_first_name + p_last_name (and
       p_minor_first_name + p_minor_last_name). The old full-name signature is
       DROPPED (clean replace); callers updated in the same change set.
    5. sign_general_release — legacy wrapper kept (trivially cheap compat): its
       p_full_name is split on the first space and forwarded.
    6. template_tokens dictionary notes updated (source honesty — cosmetic).
    7. ALTER TABLE contacts DROP COLUMN full_name (its lower(full_name) index
       drops with it); a lower(last_name), lower(first_name) index replaces it.
       No other table carries a full_name column (intake_submissions.contact_name
       is freeform anonymous input and is deliberately untouched).
*/

-- ============================================================
-- 1. Backfill: split full_name on the FIRST space where first_name is missing.
-- ============================================================
UPDATE contacts
...
-- ============================================================
-- 2. ensure_contact_for_profile — full current body (20260629020000) minus the
--    full_name column. The "born with name = email" placeholder posture is kept
--    (in first_name) so the sign_release identity heal still recognizes it.
-- ============================================================
...
BEGIN
  ...
  IF v_contact_id IS NULL THEN
    ...
    VALUES (v_first, v_last, v_profile.email, v_profile.phone,
...
-- ============================================================
-- 3. generate_document v7 — FULL v6 body (20260701080000) with ONE change: the
--    party arm's name source is the first/last concatenation (trimmed single
--    space when a part is missing) for FULL_NAME / PRINTED_NAME.
-- ============================================================
...
COMMENT ON FUNCTION generate_document(uuid, text) IS
  'Phase 3 merge engine (resolver v7, first/last-name canon): v6 (DIR arm + module fees) with the party arm''s FULL_NAME/PRINTED_NAME resolved from first_name || '' '' || last_name (contacts.full_name removed). {{SIG.*}} left for signing.';

-- ============================================================
-- 4. sign_release — signature CHANGE: split name fields. The old full-name
--    signature is dropped (clean replace; kiosk + wrapper updated with it).
-- ============================================================
DROP FUNCTION IF EXISTS sign_release(text, text, text, text, text, boolean, text, date, text, boolean, uuid);
...
BEGIN
  ...
  IF coalesce(p_is_minor, false) THEN
    ...
    IF v_minor_c IS NULL THEN
      INSERT INTO contacts (org_id, first_name, last_name)
        VALUES (v_org, v_minor_first, NULLIF(v_minor_last, ''))
...
COMMENT ON FUNCTION sign_release(text, text, text, text, text, text, boolean, text, text, date, text, boolean, uuid) IS
  'Public release kiosk (first/last-name canon): signs any of the four RELEASE_* templates. Adult path: signer = PARTICIPANT party. Minor path: minor = PARTICIPANT (not a signer), guardian = GUARDIAN party signs; DOB + relationship merged. The typed signature must match first_name || '' '' || last_name. The inapplicable signer section is stripped by marker; the completed signature + a dated Facility Rules acknowledgment are merged into the executed body. Unilateral: EXECUTES on the single signature. Requires p_rules_acknowledged.';

-- ============================================================
-- 5. sign_general_release — legacy wrapper kept (trivially cheap compat): the
...
COMMENT ON FUNCTION sign_general_release(text, text, text, text, uuid) IS
  'Legacy wrapper over sign_release(RELEASE_GENERAL, …, adult path). Splits its single full-name argument on the first space into first/last (first/last-name canon, 20260702090000). Unilateral since 20260702050000: EXECUTES on the visitor signature (no COMPANY countersign).';

-- ============================================================
-- 6. Dictionary honesty: PARTY name tokens now source from first/last (cosmetic
--    — dictionary rows document the merge; the resolver is the authority).
-- ============================================================
UPDATE template_tokens
   SET source_column = 'first_name/last_name',
       notes = CASE field
         WHEN 'FULL_NAME'    THEN 'Full Legal Name / Name — first_name || '' '' || last_name (contacts.full_name removed 20260702090000)'
         WHEN 'PRINTED_NAME' THEN 'signature block printed name — first_name || '' '' || last_name (contacts.full_name removed 20260702090000)'
         ELSE notes END
 WHERE source_table = 'contacts' AND source_column = 'full_name'
   AND field IN ('FULL_NAME', 'PRINTED_NAME');

-- ============================================================
-- 7. Drop the column (its lower(full_name) index drops with it); replace the
--    name-ordering index with a first/last one.
-- ============================================================
ALTER TABLE contacts DROP COLUMN IF EXISTS full_name;
CREATE INDEX IF NOT EXISTS contacts_name_idx ON contacts (lower(last_name), lower(first_name));
