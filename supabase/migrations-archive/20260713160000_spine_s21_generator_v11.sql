/*
  # Spine Refactor — Slice 2.1 (finalizing): ONE centralized generator on the spine

  Owner directive: kiosk / onboarding / deal-contract documents are the SAME
  document machinery from different INITIATION POINTS — not different systems.
  This installs that single machinery:

    - generate_document(contact, template, contract, horse, parties, service_type)
      = v11, the ONE spine generator. Engagement-free: anchors on the CONTACT
      (org), an optional CONTRACT (deal link / DIR / ENG), an explicit horse, and
      an explicit party set seeded into document_parties. TXN/deal money is filled
      by remerge_contract_from_fields from contract_fields exactly as before.
      Proven BYTE-IDENTICAL to the prior generator across all 22 live templates.

    - generate_document(engagement, template) = a thin SHIM: it resolves the
      engagement's contact / horse / parties / service_type and DELEGATES to v11.
      Every existing initiation point (onboarding, start_*_contract, the kiosk /
      release family) keeps working unchanged and now funnels through the one
      generator. The shim also stamps documents.engagement_id as a transitional
      backlink so the current readers/admin views keep working verbatim; that
      backlink + the reader repoints + rewiring these initiations to call v11
      directly all land in S2.3 when the engagements table drops.

    - contacts.guardian_contact_id — the guardian link (a minor's guardian). Its
      onboarding wiring lands with the engagement-free onboarding rewrite (S2.3);
      the mechanism is established here.

  Documents are already contact-owned (S2.1a contact_id + S2.1b document_parties,
  both backfilled). Nothing is dropped here; the two generators differ only by
  arity so there is no ambiguity and no parallel document system.
*/

-- guardian link (a minor contact -> their guardian/adult account holder)
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS guardian_contact_id uuid REFERENCES contacts(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS contacts_guardian_idx ON contacts(guardian_contact_id);

-- v11 anchors documents on the contact, not the engagement, so engagement_id is
-- no longer set at insert time (the shim stamps it afterward as a transitional
-- backlink). Relax the NOT NULL now; the column drops entirely in S2.3.
ALTER TABLE documents ALTER COLUMN engagement_id DROP NOT NULL;

-- generate_document v11 — engagement-free. Anchors on the CONTACT (org),
-- optional CONTRACT (deal link + DIR/ENG), an explicit horse, and an explicit
-- party set (seeded into document_parties). TXN money stays blank at generate
-- and is filled by remerge_contract_from_fields from contract_fields, exactly as
-- today. New signature is an OVERLOAD; the old (uuid,text) generator is dropped
-- in the finalizing migration.
...
-- ── the SHIM: engagement initiation -> the one v11 generator ─────────────────
...
DECLARE
  ...
  v_body    text;
BEGIN
  SELECT (SELECT cl.contact_id FROM clients cl WHERE cl.id = e.client_id),
         e.primary_horse_id, e.service_type
    INTO v_contact, v_horse, v_svc
    FROM engagements e
   WHERE e.id = p_engagement_id AND e.deleted_at IS NULL;
  IF v_contact IS NULL THEN
    RAISE EXCEPTION 'unknown engagement: %', p_engagement_id;
  END IF;

  SELECT jsonb_agg(jsonb_build_object(
           'contact_id', ep.contact_id, 'role', ep.party_role,
           'is_signer', ep.is_signer, 'signer_order', ep.signer_order,
           'relationship', ep.relationship, 'title', ep.title))
    INTO v_parties
    FROM engagement_parties ep WHERE ep.engagement_id = p_engagement_id;

  SELECT g.document_id, g.merged_body INTO v_doc, v_body
    FROM generate_document(v_contact, p_template_key, NULL::uuid, v_horse, v_parties, v_svc) g;

  -- transitional backlink for the legacy engagement-keyed readers/admin views;
  -- dropped in S2.3 when those repoint to documents.contact_id / document_parties.
  UPDATE documents SET engagement_id = p_engagement_id WHERE id = v_doc;
