/*
  # Vet Auth — euthanasia A/B authorization + horse capacity

  - Euthanasia: replaces the free "CLIENT Initials" line with a REQUIRED,
    mutually-exclusive A/B authorization the OWNER makes (a leased horse's vet
    docs are owner-signed). Stored on horses.euthanasia_authorization ('A'|'B');
    the body shows both options with an [X] on the chosen one.
      A = authorize humane euthanasia in the vet's judgment when unreachable.
      B = do NOT authorize without express consent; reach owner/emergency first.
  - Horse capacity: {{CLIENT.HORSE_CAPACITY}} now renders the signer's basis of
    authority, derived from the horse (owner→"owns", lessee→"leases", else
    "is an authorized agent of").

  Recreates generate_document (adds HORSE.EUTHANASIA_A/B + CLIENT.HORSE_CAPACITY),
  create_horse_record (writes euthanasia_authorization), patches the vet-auth
  body, re-registers its tokens.
*/

ALTER TABLE horses
  ADD COLUMN IF NOT EXISTS euthanasia_authorization text
  CHECK (euthanasia_authorization IS NULL OR euthanasia_authorization IN ('A','B'));
...
$function$
;
REVOKE ALL ON FUNCTION public.generate_document(uuid, text, uuid, uuid, jsonb, text) FROM public;
...
-- ── Vet Auth body: replace the free-initials line with the required A/B block ──
UPDATE contract_templates SET body = replace(
    body,
    E'CLIENT Initials: {{CLIENT.EUTHANASIA_INITIALS}}',
 ...
 WHERE template_key = 'HORSE_EMERGENCY_VET'
   AND body LIKE '%{{CLIENT.EUTHANASIA_INITIALS}}%';

-- re-register vet-auth tokens (drops the retired EUTHANASIA_INITIALS, adds A/B)
DELETE FROM template_tokens
 WHERE template_id = (SELECT id FROM contract_templates WHERE template_key = 'HORSE_EMERGENCY_VET');
INSERT INTO template_tokens (template_id, namespace, field, token, kind, required, party_scoped)
  SELECT (SELECT id FROM contract_templates WHERE template_key='HORSE_EMERGENCY_VET'),
    ...
    FROM (SELECT DISTINCT unnest(regexp_matches(
            (SELECT body FROM contract_templates WHERE template_key='HORSE_EMERGENCY_VET'),
            '\{\{[A-Z0-9_.]+\}\}', 'g')) AS tok) t;
