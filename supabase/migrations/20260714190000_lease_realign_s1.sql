/*
  # Lease realign · Slice 1 — operational terms become contract fields

  The lease's operational terms (scheduling/availability, lessons-per-level,
  exclusivity, events, sublease/shared permission) move ONTO the lease contract
  as structured `contract_fields`, so they inherit the contract workflow:
  authored by the responsible party, owner-owned where it's the owner's call,
  staff-editable, and frozen once signed.

  Changes to start_lease_contract:
   - `p_responsible_role` ('LESSEE' | 'LESSOR', default 'LESSEE') sets the
     document ORIGINATOR — so the responsible party owns/edits the DEAL terms.
   - Appended operational fields: scheduling (DAYS_USED, DAYS_UNAVAILABLE),
     lessons-per-level (LESSONS_BEGINNER/INTERMEDIATE/ADVANCED), EXCLUSIVITY_RULES,
     EVENTS_AUTHORIZED, PAYMENT_OPTIONS — all DEAL-owned; and SUBLEASE_ALLOWED,
     SHARED_LEASE_ALLOWED as LESSOR-owned (the owner's sole discretion).

  contract_fields is empty in production, so re-seeding is clean. All prior
  field payloads are spliced verbatim.
*/
DROP FUNCTION IF EXISTS start_lease_contract(uuid, uuid, uuid);
