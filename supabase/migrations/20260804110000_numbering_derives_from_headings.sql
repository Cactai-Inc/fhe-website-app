-- R11 PHASE A2 (owner ruling, 2026-08-04) — NUMBERING DERIVES FROM HEADINGS.
--
-- A number in an executed instrument is an ENFORCEABLE cross-reference. It may
-- therefore exist only where there is a titled thing to reference: a HEADER —
-- a clause carrying its own heading. The defect this migration fixes is that
-- remerge_contract_from_clauses incremented v_cl_no for EVERY included clause
-- and prefixed '§CLAUSENUM§.<n>' onto the first body line of a HEADINGLESS
-- clause, manufacturing numbered items that title nothing:
--
--     3.3 Are there any ownership related leasing restrictions? No.
--     3.10 Lessor may inspect the Horse at any time, …
--     10.6 Lessor does not restrict competition activity in any way.
--
-- The rule, applied here and mirrored in src/components/app/ClauseDocument.tsx:
--   • clause WITH a heading      → consumes the next sub-number and emits
--                                  "§CLAUSENUM§.<m> <heading>" then its body
--   • clause WITHOUT a heading   → CONTINUATION of the item above it: its body
--                                  lines are emitted with NO number and NO
--                                  increment. Headingless clauses preceding the
--                                  first header in a section are that section's
--                                  preamble under "N. TITLE", unnumbered.
--   • gated-off clause           → already skipped above (never numbered).
-- render_as_subitem rows keep their existing append-to-previous behaviour.
-- Numbers stay order-derived, so insertion/removal renumbers automatically.
--
-- REWRITE-IN-PLACE CAVEAT: like ~31 other migrations in this repo, this file
-- replaces a live function body; it is not meant to be replayed onto a fresh
-- database. The full pre-change body is recorded in docs/reports/TASK-R11-REPORT.md.
