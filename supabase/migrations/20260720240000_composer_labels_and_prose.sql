-- Composer fixes surfaced by the screenshots review:
--   #1  A clause line whose only token is empty was suppressed even when the line
--       carries substantive prose. That dropped §3.2/§3.4 (now pure-prose
--       warranties). Fix: only suppress a token-bearing line when, after removing
--       every token, no real words remain (i.e. it was purely "Label: {{blank}}").
--   #2  select / yes-no tokens substituted the STORED CODE ("GELDING", "ENABLED")
--       instead of the option LABEL ("Gelding", "Evaluation period required").
--       Fix: substitute each token with its display label via a value→label map
--       built from the template's field_defs (yes-no → Yes/No).

-- Map a token's stored value to what should PRINT in the document: an option's
-- label, "Yes"/"No" for a yes/no field, or the raw value otherwise. v_labels is a
-- jsonb of { field_key: { value: label, ... } } assembled per template.
CREATE OR REPLACE FUNCTION public.token_display_value(
  p_token text, p_raw text, p_labels jsonb
) RETURNS text
 LANGUAGE sql IMMUTABLE
AS $$
  SELECT CASE
    WHEN coalesce(p_raw,'') = '' THEN ''
    -- explicit option label for this field+value, if one exists
    WHEN p_labels #>> ARRAY[p_token, p_raw] IS NOT NULL
      THEN p_labels #>> ARRAY[p_token, p_raw]
    -- yes/no fields carry no options; normalize the code to a word
...
DECLARE
  ...
  v_labels     jsonb := '{}'::jsonb;   -- { field_key: { value: label } } for select/option fields
  v_out        text[] := '{}';
  v_sec_buf    text[];
  v_sec        record;
  v_cl         record;
...
BEGIN
  ...
  -- value→label map for every option-bearing field on this template
  SELECT coalesce(jsonb_object_agg(field_key, vmap), '{}'::jsonb) INTO v_labels
  FROM (
    SELECT field_key,
           jsonb_object_agg(opt->>'value', opt->>'label') AS vmap
      FROM contract_field_defs fd
      CROSS JOIN LATERAL jsonb_array_elements(fd.options) AS opt
     WHERE fd.template_key = v_tkey AND fd.options IS NOT NULL
     GROUP BY field_key
  ) m;
    ...
    LOOP
      ...
      IF v_body <> '' THEN
        ...
        FOREACH v_line IN ARRAY v_lines LOOP
          ...
          -- #1: a line with empty tokens is dropped ONLY when it is purely a
          -- "Label: {{blank}}" fragment — i.e. once every token is stripped, no
          -- real words remain. A line with substantive prose always stays (its
          -- blanks just render empty, matching a paper form's blank line).
          ...
          FOREACH v_tok IN ARRAY v_toks LOOP
            ...
            ELSE
              -- #2: print the option LABEL (or Yes/No), not the stored code
              v_line := replace(v_line, '{{'||v_tok||'}}',
                token_display_value(v_tok, v_fields ->> v_tok, v_labels));
            END IF;
          END LOOP;
