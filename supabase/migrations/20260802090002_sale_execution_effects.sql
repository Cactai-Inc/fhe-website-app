/*
  # Sale execution effects — HORSE_SALE / HORSE_BILL_OF_SALE kinds

  Extends apply_contract_execution_effects (trigger on documents.workflow_state →
  'executed') to the clause-model sale kinds, resolved via contract_kind — never
  the literal template_key (the v1/v2 lesson the classifier column encodes):

  - HORSE_SALE and the legacy HORSE_PURCHASE_SALE run the ownership-transfer
    branch: end the active OWNER relationships (the prior owner stays recorded
    with ended_at — the audit trail), transfer current_owner_contact_id to the
    buyer, clear lease fields, insert the new OWNER relationship.
  - HORSE_BILL_OF_SALE runs the same transfer ONLY standalone
    (TXN.BOS_HAS_SALE_AGREEMENT <> 'YES'); when a sale agreement accompanies it,
    the sale document drives the transfer and the bill of sale has no effect.
  - Conservative default (flagged decision): home location is NOT changed on
    transfer — staff get a dashboard notification to review it.
*/
