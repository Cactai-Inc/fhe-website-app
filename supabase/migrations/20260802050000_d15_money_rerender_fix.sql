-- ============================================================================
-- D15 FIX — remerge_contract_from_clauses missing money formatting.
--
-- Verified before this fix (owner instruction: verify first, don't assume):
-- document c36449f7-a29f-4b12-9313-4f9a8a0ca9a1's HORSE.FAIR_MARKET_VALUE
-- rendered as "52500.00" (bare number) both before and after a real field
-- edit (HORSE.CURRENT_LOCATION, via set_contract_field) triggered a real
-- re-merge (via remerge_contract_body -> remerge_contract_from_clauses).
--
-- Root cause: generate_document (document creation) calls fmt_money() for
-- money fields (FAIR_MARKET_VALUE, CANCELLATION_FEE, LATE_FEE, NO_SHOW_FEE,
-- commission fields). remerge_contract_from_clauses (the re-render path used
-- on every subsequent field edit) has NO money branch at all — it already
-- special-cases 'certify' and 'percent' format_type fields, but falls
-- through every other field (including 'currency') to token_display_value,
-- which has no money handling either. One token (a field with
-- format_type='currency'), one source (fmt_money) — the same function
...
-- captured 2026-08-02). The ONLY change is the new currency branch, inserted
-- as a sibling to the existing percent branch (same shape: numeric-value
-- guard + a format_type lookup scoped to template_key + field_key).
--
-- Scope, verified: only 2 live fields carry format_type='currency'
-- (HORSE_LEASE_V2.HORSE.FAIR_MARKET_VALUE, HORSE_LEASE_V2.TXN.EVAL_FIXED_FEE).
-- TXN.LEASE_FEE's format_type='fee_schedule' is a distinct, more complex
-- JSON-schedule format handled elsewhere (not a plain {{TOKEN}} substitution,
-- confirmed absent from generate_document too) — deliberately NOT touched by
-- this fix; out of scope per the verified defect.
-- ============================================================================
