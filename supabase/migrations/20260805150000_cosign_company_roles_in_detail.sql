/*
  # TASK COSIGN — expose company-signable roles from contract_document_detail

  Read-model only. record_signature() already lets a staff caller sign a party
  role when that role's signer contact is the org's own company contact
  (20260803020000_company_side_signing.sql) — proven live in TASK-A16. The gap
  is entirely in the UI: contract_document_detail()'s `my_roles` is derived
  from caller_party_roles(), which matches the CALLER's own contact_id — a
  staff caller whose contact isn't itself a party never receives the
  company's roles, so ContractPage has no reliable, labeled way to offer
  "sign on behalf of the company" versus the pre-existing generic "sign on a
  party's behalf" box (added 20260722, before 20260803's tightening), which
  still renders for EVERY pending non-mine role including individual parties
  that record_signature now rejects.

  Adds two read-only fields to the payload, computed with the exact same
  join record_signature uses for its company branch (mirrored, not invented):
    - company_signable_roles: text[] — party roles on this doc whose signer
      contact is org.is_company (the roles a staff caller may sign).
    - company_contact_name: text | null — the company contact's display name,
      for the UI label ("Sign as <name>").

  Body otherwise carried forward unchanged from the live definition.
*/
