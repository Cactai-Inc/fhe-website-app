-- A11: lessee-perspective framing on the horse page needs to know whether the
-- viewer IS the lessee (not just the resolved lessee's name, which is all the
-- payload carried before). current_contact_id() isn't exposed to the client
-- for comparison, so the RPC compares server-side and returns the verdict —
-- a jsonb payload addition, no signature change. Full body carried forward
-- from live otherwise unchanged (supabase/migrations/…horse_record_system.sql
-- and its later revisions).
