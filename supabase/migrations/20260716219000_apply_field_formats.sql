-- apply_field_formats(doc) — assigns format_type to every field from its semantics
-- and links the manage↔cost pairs. Idempotent. Called by the lease seeder so NEW
-- leases get the structured/paired model, and reusable as a backfill for existing docs.
-- (Previously these assignments only ran as one-off migrations against existing rows.)
...
BEGIN
  ...
  UPDATE contract_fields SET format_type='phone'       WHERE document_id=p_document_id AND field_key LIKE '%.PHONE';
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.apply_field_formats(uuid) TO authenticated;
