-- U2.3/U2.4/U2.5 — generate_document rebuilt as a full CREATE OR REPLACE from
-- its LIVE pg_get_functiondef body (2026-08-01). Three changes only:
--   U2.3 MEDICATION_DOSAGE/_INSTRUCTIONS/_ADDITIONAL: hardcoded '' -> per-
--        component resolvers (horse_medications_prose has no mode argument;
--        it renders the whole line, so reusing it would triple the text).
--   U2.4 DOC.EFFECTIVE_DATE: to_char(now()) -> coalesce(effective_date,
--        created_at::date), the SAME source remerge_contract_from_clauses
--        uses at line 81. One token, one truth.
--   U2.5 location tokens: locations.name alone -> location_full_label(),
--        which composes the facility address.
...
BEGIN
  ...
  -- so the expander below sees every horse on this one pass. One id (or
  -- none) changes nothing — the single-horse path is untouched.
