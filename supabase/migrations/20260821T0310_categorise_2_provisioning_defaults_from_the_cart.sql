-- TASK-CATEGORISE §2 — THE DERIVED SET REACHES PROVISIONING.
--
-- Task doc §2: "`provision_client_invitation`'s `p_categories` must DEFAULT from
-- the request's derived set when provisioning from a request, rather than being
-- chosen by hand from the funnel." And: "Staff must still be able to override.
-- The derivation is the default, not a cage."
--
-- The default lands IN THE DATABASE and not only in the form, because
-- `ProvisionClientForm` is not the only caller: `/api/sign-start` calls this RPC
-- too, and CLOSEOUT §3.4 already settled the principle in this repo's own words —
-- "client-side-only enforcement is a request, not a rule."
--
-- ⚠️ REISSUED FROM THE LIVE BODY (`pg_get_functiondef`, 2026-08-21), not from the
-- latest migration file that touches it. Two edits, and nothing else moves:
--   1. the "at least one category is required" guard moves BELOW the org
--      resolution (the derivation needs v_org; nothing between reads v_cats),
--   2. the derive-when-empty block is inserted at its new home.
-- Everything else — the LESSONREQUEST §L3 agreed-lesson block, §C5b order
-- confirmation, the INVITEWORKS supersession, apply_affiliations — is byte-for-
-- byte what was live.
