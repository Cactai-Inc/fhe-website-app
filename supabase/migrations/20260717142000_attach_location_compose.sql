-- HORSE.HOME_LOCATION / CURRENT_LOCATION tokens now compose name + structured
-- address + this horse's barn/stall, so the contract shows a findable location.
...
BEGIN
  ...
  v_home_loc := nullif(btrim(concat_ws(' — ',
    ...
    nullif(btrim(v_horse.home_barn_stall),''))), '');
  v_curr_loc := nullif(btrim(concat_ws(' — ',
    ...
    nullif(btrim(v_horse.current_barn_stall),''))), '');
