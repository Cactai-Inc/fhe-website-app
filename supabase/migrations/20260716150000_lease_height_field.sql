-- Fix dead lease field: HORSE.HEIGHT rendered in the body but was never seeded
-- as an editable/fillable contract_field, so it showed as a blank with no input.
-- Added it to start_lease_contract's seeded fields (fills from the horse record
-- like the other HORSE.* fields). SIG.* tokens fill on signing; TXN.ADDITIONAL_TERMS
-- fills from accepted clauses — those are not dead.
...
BEGIN
  ...
  -- The responsible party authors the deal terms â they are the originator.
  ...
  v_n := seed_contract_fields(v_doc, jsonb_build_array(
    ...
    -- ââ Operational scheduling terms (drive the leased-horse calendar) ââ
    jsonb_build_object('field_key','TXN.PAYMENT_OPTIONS','label','Payment Options (one per line: amount â describe)','section','Payment','owner_role','DEAL','value_type','longtext','sort_order',415),
    ...
    jsonb_build_object('field_key','TXN.LESSONS_BEGINNER','label','Lessons/Day â Beginner','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',820),
    jsonb_build_object('field_key','TXN.LESSONS_INTERMEDIATE','label','Lessons/Day â Intermediate','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',830),
    jsonb_build_object('field_key','TXN.LESSONS_ADVANCED','label','Lessons/Day â Advanced','section','Scheduling & Availability','owner_role','DEAL','value_type','number','sort_order',840),
    ...
    -- ââ Owner-only permissions (lessor's sole discretion) ââ
