-- DOCPACKET (owner, 2026-08-10): the onboarding set reads as one packet in the
-- Clients (/app/admin) Documents tab, not N individual rows. Presentation only —
-- storage, generation and signing are untouched.
--
-- The FE needs to know which rows are onboarding-class (wall_gating) to group
-- them into the packet, so admin_client_documents grows one column. No new
-- table, no new relationship — the class is already knowable from
-- contract_templates.wall_gating, same source my_wall_state() reads.
DROP FUNCTION IF EXISTS public.admin_client_documents(uuid);

CREATE FUNCTION public.admin_client_documents(p_user_id uuid)
 RETURNS TABLE(id uuid, title text, status text, workflow_state text, created_at timestamp with time zone, wall_gating boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  -- generated documents (the real instances)
  SELECT d.id, d.title, d.status, d.workflow_state, d.created_at, coalesce(ct.wall_gating, false)
  FROM documents d
