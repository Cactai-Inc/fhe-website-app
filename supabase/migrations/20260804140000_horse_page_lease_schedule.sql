...
-- Read-through, not copy: no new columns/tables, no stamping. The schedule
-- (TXN.DAYS_USED / TXN.SCHEDULE_TERMS) lives only in the executed lease
-- document's contract_fields; this reads it at display time. Gate on the
-- horses lease-window stamp (execution-time truth, kept fresh by
-- apply_contract_execution_effects) then pull the latest matching executed
-- lease document for that horse via the same documents.horse_id linkage the
-- execution trigger uses. Full body carried forward from
-- 20260804130000_horse_page_viewer_is_lessee.sql otherwise unchanged.
...
AS $function$
DECLARE v_h horses%ROWTYPE; v_out jsonb; v_lease jsonb;
BEGIN
  ...
  -- A12: the horses stamp is the execution-time truth for whether a lease is
  -- currently active; only look for a source document when it says so.
  IF v_h.lease_start IS NOT NULL AND v_h.lease_start <= current_date
     AND (v_h.lease_end IS NULL OR v_h.lease_end >= current_date) THEN
    SELECT jsonb_build_object(
        ...
        'lease_type', nullif(cf.lease_type, ''),
        'days_used', nullif(cf.days_used, ''),
        'schedule_terms', nullif(cf.schedule_terms, ''),
        'source_document_id', d.id
      ) INTO v_lease
      FROM documents d
      JOIN contract_templates ct ON ct.id = d.template_id
      CROSS JOIN LATERAL (
        SELECT max(value) FILTER (WHERE field_key = 'TXN.LEASE_TYPE') AS lease_type,
               max(value) FILTER (WHERE field_key = 'TXN.DAYS_USED') AS days_used,
               max(value) FILTER (WHERE field_key = 'TXN.SCHEDULE_TERMS') AS schedule_terms
          FROM contract_fields WHERE document_id = d.id
      ) cf
      WHERE d.horse_id = p_horse_id AND d.deleted_at IS NULL
        AND d.workflow_state = 'executed' AND is_horse_lease_template(ct.template_key)
      ORDER BY d.effective_date DESC NULLS LAST, d.created_at DESC
      LIMIT 1;
  END IF;
  ...
  v_out := jsonb_build_object(
    ...
    -- A11: true only when the signed-in viewer IS the stamped lessee (owner/staff
    -- always get the outward "Leased to <name>" framing, never this).
    'viewer_is_lessee', (v_h.lessee_contact_id IS NOT NULL AND v_h.lessee_contact_id = current_contact_id()),
    -- A12: schedule captured in the executed lease contract, read through at
    -- display time. Absent (not a key with null value) when no active lease.
    'lease', v_lease,
