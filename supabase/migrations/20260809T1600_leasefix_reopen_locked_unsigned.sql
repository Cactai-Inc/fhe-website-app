/*
  # LEASEFIX 2f — a locked, unsigned document can be reopened

  Found while checking what happens when one party has signed and the other wants
  a change. That case is FINE and already wired: the signature makes the document
  read-only, the non-signer gets "Request permission to edit"
  (request_permission_to_edit), the signer gets "Remove my signature"
  (remove_my_signature), and withdrawal archives the attestation to audit_logs,
  regresses status to AWAITING_SIGNATURE and returns workflow_state to editable.

  The BROKEN case is the one before any signature exists. From `locked` the only
  legal transitions were `void` and `executed`:

      locked -> editable    illegal transition
      locked -> editing     illegal transition
      locked -> in_review   illegal transition

  and remove_my_signature returns early ("no standing signature to remove") without
  touching workflow_state. So a document locked by mistake could only be VOIDED.
  The staff "Withdraw / correct" button renders in precisely that state
  (locked, nothing signed) and raised on every click.

  This adds the one missing edge, guarded twice: staff only, and refused outright
  once any signature stands — with a signature the escape remains withdrawal, so
  no attestation is ever discarded silently.
