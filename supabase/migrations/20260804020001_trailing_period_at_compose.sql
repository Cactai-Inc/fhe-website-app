-- remerge_contract_from_clauses: append sentence-terminal punctuation at
-- COMPOSE time (R5, owner directive 2026-08-04). Bodies no longer carry a
-- trailing "." after an input token; the composer adds one when the resolved
-- line needs it, so a signer's own period never doubles and an empty value
-- never leaves an orphan. Body carried forward from live otherwise unchanged.
