/*
  # FHE Suite — Release kiosk generalization (all four releases, owner directives 2026-07-02)

  The public /release kiosk grows from RELEASE_GENERAL-only into the full
  release surface. Owner directives implemented here:

  1. ALL FOUR releases sign at the kiosk — RELEASE_GENERAL, RELEASE_PARTICIPANT,
     RELEASE_HORSE_EXERCISE, RELEASE_HORSE_CARE.
  2. Minor flow — a "minor involved" path where the PARENT/GUARDIAN signs: the
     minor is recorded as the PARTICIPANT party (is_signer = false), the guardian
     as the GUARDIAN party (is_signer = true); the signed document carries the
     minor section (minor name, DOB, guardian name + relationship, guardian
     signature) and NOT the adult section — and vice versa for adults.
  3. Signature blocks are NEVER shown pre-signing — release_preview truncates the
     body at the first signer-section marker; the signed document carries the
     COMPLETED signature section (typed name + date merged in).
  4. RELEASES ARE UNILATERAL — the COMPANY countersignature block was removed
     from all four templates (owner decision 2026-07-02); no COMPANY signer party
     is attached, so the document EXECUTES on the single signature. The company
     is identified in the party block by {{ORG.LEGAL_NAME}} (trade name ONLY —
     no personal name / LEGAL_IDENTITY on releases, owner decision 2026-07-02).
  5. Rules gate — the kiosk shows FACILITY_RULES (merged, truncated) behind a
     required checkbox before the release page; sign_release REQUIRES
     p_rules_acknowledged = true and merges a dated Facility Rules acknowledgment
     line into the executed body.

  MARKER SCHEME (shared by templates, preview truncation, and section stripping):
    'ADULT SIGNER'                    — start of the adult signature section
    'MINOR SIGNER (PARENT/GUARDIAN)'  — start of the minor/guardian section
  Both sections exist in every RELEASE_* template, adult first, minor last (no
  COMPANY block follows). FACILITY_RULES keeps its own signature block; its
  preview truncates at the CLIENT signature marker.

  Back-compat wrappers: general_release_preview(p_org) and
  sign_general_release(...) delegate to the new fns with RELEASE_GENERAL.
  KNOWN BEHAVIOR CHANGE (deliberate, owner rule 4): sign_general_release now
  EXECUTES immediately (no COMPANY countersign party). The legacy wrapper
  forwards p_rules_acknowledged = true — it predates the rules gate and is
  retained only for pipeline compatibility; the live kiosk always gates.

  Org resolution (both fns, unchanged from 20260702020000/20260702040000):
    coalesce(p_org, current_org(), current_addressed_org(), sole_org()).
*/

-- ============================================================
-- 0. Template metadata — the releases' signing parties are PARTICIPANT +
--    GUARDIAN across all four (unilateral: COMPANY is prose, not a party;
--    the horse releases' signer block moved OWNER → PARTICIPANT).
-- ============================================================
UPDATE contract_templates
   SET party_namespaces = ARRAY['PARTICIPANT','GUARDIAN']
 WHERE template_key IN ('RELEASE_GENERAL','RELEASE_PARTICIPANT',
                        'RELEASE_HORSE_EXERCISE','RELEASE_HORSE_CARE');

-- ============================================================
-- 1. release_preview — any of the four releases (or the FACILITY_RULES gate
--    document), merged org identity + dates, TRUNCATED before the signature
--    area. Generalizes 20260702040000_general_release_preview.
-- ============================================================
...
REVOKE ALL ON FUNCTION sign_release(text, text, text, text, text, boolean, text, date, text, boolean, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sign_release(text, text, text, text, text, boolean, text, date, text, boolean, uuid)
  TO anon, authenticated, service_role;

COMMENT ON FUNCTION sign_release(text, text, text, text, text, boolean, text, date, text, boolean, uuid) IS
  'Public release kiosk: signs any of the four RELEASE_* templates. Adult path: signer = PARTICIPANT party. Minor path: minor = PARTICIPANT (not a signer), guardian = GUARDIAN party signs; DOB + relationship merged. The inapplicable signer section is stripped by marker; the completed signature + a dated Facility Rules acknowledgment are merged into the executed body. Unilateral: EXECUTES on the single signature. Requires p_rules_acknowledged.';

-- Back-compat: the original RPC delegates to sign_release (adult path,
-- RELEASE_GENERAL). BEHAVIOR CHANGE per owner rule 4: EXECUTES immediately —
-- no COMPANY countersign party. p_rules_acknowledged is forwarded true: this
-- legacy seam predates the rules gate and remains only for pipeline
-- compatibility; the live kiosk gates on the checkbox and calls sign_release.
...
AS $fn$
  SELECT sign_release('RELEASE_GENERAL', p_full_name, p_email, p_phone, p_typed_name,
                      false, NULL, NULL, NULL, true, p_org);
$fn$;
...
COMMENT ON FUNCTION sign_general_release(text, text, text, text, uuid) IS
  'Legacy wrapper over sign_release(RELEASE_GENERAL, …, adult path). Unilateral since 20260702050000: EXECUTES on the visitor signature (no COMPANY countersign).';
