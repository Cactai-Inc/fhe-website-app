-- attach_horse_to_document now maps MARKINGS/REGISTRATION_ORG/PASSPORT_NUMBER/
-- PASSPORT_COUNTRY (captured at intake but previously had no HORSE.* token).
...
BEGIN
  ...
  LOOP
    v_val := CASE r.field
      ...
      WHEN 'REGISTRATION_ORG'    THEN v_horse.registration_org
      WHEN 'PASSPORT_NUMBER'     THEN v_horse.passport_number
      WHEN 'PASSPORT_COUNTRY'    THEN v_horse.passport_country
