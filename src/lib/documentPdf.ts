 ...
 *
 * ⚠️ THIS FILE HAD SILENTLY DRIFTED (found 2026-08-24). The docstring above said
 * "same pdf-lib layout" while this copy still used a 0.75" margin and had NONE of
 * the keep-a-heading-with-its-content / keep-the-signature-with-the-closing-text
 * rules the server copy grew. The same document downloaded in-app and received by
 * email paginated differently. The layout below is now a verbatim port of the
 * server one; the two tsconfig projects (`api` and `src`) share no module, so
...
const PAGE_H = 792;
/* Keep IN STEP with api/_lib/documentPdf.ts — see the note at the top of this
   file. 0.64", reduced from 0.75" on 2026-08-24. */
const MARGIN = 46;
const FONT_SIZE = 10;
const LINE_H = 14;
