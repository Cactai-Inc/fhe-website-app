  ...
  try {
    ...
    if (event.type === 'checkout.session.completed' || event.type === 'payment_intent.succeeded') {
      ...
      if (orderId) {
        ...
        // for one checkout) — an already-paid purchase must not be re-paid, and
        // confirm_booking_for_purchase must not run twice. Guards on 'paid'
        // because that is what mark_purchase_paid writes, so a purchase paid by
        // any route (Stripe, Zelle, staff) is caught here.
        ...
        // retired): mark it paid via RPC, then confirm the held booking.
        // mark_purchase_paid sets both status='paid' and paid_at internally —
        // re-writing status here produced a Stripe-only 'confirmed' that no
        // other payment route ever reached.
