...
beforeAll(async () => {
  ...
  await h.q(`insert into contacts (full_name, org_id) values ('A Contact',$1)`, [orgA]);
  await h.q(`insert into contacts (full_name, org_id) values ('B Contact',$1)`, [orgB]);
...
describe('tenant isolation', () => {
  it('each tenant sees its own contacts and not the other’s', async () => {
    ...
    const a = (await h.q<{ full_name: string }>(`select full_name from contacts`)).map((r) => r.full_name);
    expect(a).toContain('A Contact');
    expect(a).not.toContain('B Contact');

    await h.asUser(bStaff);
    const b = (await h.q<{ full_name: string }>(`select full_name from contacts`)).map((r) => r.full_name);
  ...
  it('a tenant cannot write a row into another tenant', async () => {
    ...
    await expect(h.q(`insert into contacts (full_name, org_id) values ('X',$1)`, [orgB]))
      .rejects.toThrow();
  });

  it('inserts default org_id to the caller’s tenant', async () => {
    await h.asUser(aStaff);
    await h.q(`insert into contacts (full_name) values ('Defaulted')`); // no org_id given
    await h.asSuperuser();
    const [row] = await h.q<{ org_id: string }>(`select org_id from contacts where full_name='Defaulted'`);
